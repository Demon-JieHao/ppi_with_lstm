...
def cnn_lstm_model(conv_layers,
                   filters,
                   size_kernel,
                   pooling_multiplier,
                   global_pooling,
    ...
    bi_lstm = keras.layers.Bidirectional(
        keras.layers.CuDNNLSTM(units=lstm_units))
    ...
    # Create the last convolutional layer
    convolutions[conv_layers - 1] = keras.layers.Conv1D(
        filters, size_kernel, activation='relu')
    ...
    for i in range(2):
        embedding[i] = bi_lstm(embedding[i])
